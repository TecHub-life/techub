...
2. Boot the full stack (web, CSS watcher, Solid Queue workers, recurring scheduler):
   ...
   **Developing background jobs:** `bin/dev` now skips Solid Queue workers and recurring schedulers
   in development unless you explicitly opt in by setting `DEV_BACKGROUND_JOBS_ENABLED=1` (or
   `true`) in your `.env`. This keeps noisy recurring workloads off by default. When you need to
   work on jobs, set the flag, restart `bin/dev`, and the `jobs` process will boot normally.
