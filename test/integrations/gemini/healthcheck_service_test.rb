...
module Gemini
  class HealthcheckServiceTest < ActiveSupport::TestCase
    ...
    test "returns success on 200" do
      Rails.application.stub :credentials, CredentialsStub.new({ google: { project_id: "proj", location: "us-central1" } }) do
        # Stub token creation and Faraday calls
        dummy_conn = Faraday.new(url: "https://us-central1-aiplatform.googleapis.com") do |f|
          f.adapter :test, Faraday::Adapter::Test::Stubs.new { |stub|
            stub.post("/v1/projects/proj/locations/us-central1/publishers/google/models/gemini-2.5-flash:generateContent") { |env| [ 200, { "content-type"=>"application/json" }, { candidates: [] } ] }
          }
        end
        ...
        Gemini::ClientService.stub :call, ServiceResult.success(dummy_conn) do
          ...
          assert_equal({ status: "ok", model: "gemini-2.5-flash" }, result.value)
        end
      end
    end
    ...
    test "returns failure on non-2xx" do
      Rails.application.stub :credentials, CredentialsStub.new({ google: { project_id: "proj", location: "us-central1" } }) do
        dummy_conn = Faraday.new(url: "https://us-central1-aiplatform.googleapis.com") do |f|
          f.adapter :test, Faraday::Adapter::Test::Stubs.new { |stub|
            stub.post("/v1/projects/proj/locations/us-central1/publishers/google/models/gemini-2.5-flash:generateContent") { |env| [ 403, { "content-type"=>"application/json" }, { error: "forbidden" } ] }
          }
        end
