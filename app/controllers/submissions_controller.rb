class SubmissionsController < ApplicationController
  ...
  def create
    ...
    # Eligibility gate (if enabled)
    if FeatureFlags.enabled?(:require_profile_eligibility)
      elig = evaluate_eligibility(profile)
      unless elig[:eligible]
        return redirect_to submit_path, alert: "@#{login} is not eligible (score #{elig[:score]}/#{elig[:threshold]})"
      end
    end

    # Ownership cap (default 5)
    if current_user.profiles.count >= (ENV["PROFILE_OWNERSHIP_CAP"].presence || 5).to_i
      return redirect_to submit_path, alert: "You have reached the maximum number of profiles"
    end

    # Link ownership
    ProfileOwnership.find_or_create_by!(user: current_user, profile: profile)
    ...
    if FeatureFlags.enabled?(:submission_manual_inputs)
      ...
      profile.update!(submitted_scrape_url: url) if url.present?
      ...
      Array(submission_params[:submitted_repositories]).compact.map(&:to_s).map(&:strip).reject(&:blank?).first(4).each do |full_name|
        ...
        pr = profile.profile_repositories.find_or_initialize_by(full_name: "#{owner}/#{repo}", repository_type: "submitted")
        pr.name ||= repo
        pr.save!
      end
    end
