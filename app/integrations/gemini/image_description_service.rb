...
module Gemini
  class ImageDescriptionService < ApplicationService
    ...
    FALLBACK_PROMPT = <<~PROMPT.squish.freeze
      Describe the subject in this image in 2-3 complete sentences.
      Mention face (if present), expression, clothing, colours, background, and overall vibe.
    PROMPT

    DEFAULT_PROMPT = <<~PROMPT.squish.freeze
      Observe the image and populate the response schema with grounded details.
      - description: 2-3 complete sentences (each ≥12 words) summarising appearance, attire, setting, and overall vibe.
      - facial_features: key facial details (hair, headwear, facial hair, eyewear) when applicable.
    ...
    def call
      ...
      return failure(StandardError.new("Image path is blank")) if image_path.blank?

      resolved_path = resolve_path(image_path)
      return failure(StandardError.new("Image not found at #{resolved_path}")) unless File.exist?(resolved_path)

      mime_type = detect_mime_type(resolved_path)
      return failure(StandardError.new("Unsupported mime type for image")) unless mime_type&.start_with?("image/")
    ...
    def response_schema
      {
        ...
        properties: {
          description: {
            type: "string",
            description: "Two to three complete sentences describing the subject."
          },
