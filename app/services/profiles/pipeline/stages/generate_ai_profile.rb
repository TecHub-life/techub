    ...
    module Stages
      class GenerateAiProfile < BaseStage
        ...
        def run_ai_traits(profile)
          trace(:started, provider: "ai_studio")
          ...
          if result.failure?
            ...
            return failure_with_context(result.error || StandardError.new("ai_traits_failed"), metadata: { upstream: safe_metadata(result) })
          end

          context.card = result.value
          trace(:completed, card_id: result.value&.id, attempts: safe_metadata(result)&.[](:attempts))
          success_with_context(result.value, metadata: { attempts: safe_metadata(result)&.[](:attempts) })
        end
